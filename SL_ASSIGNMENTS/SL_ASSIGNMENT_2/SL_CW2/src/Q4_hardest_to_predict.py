...
# def train_weights_store_misclass(train_new_weights, ds, d_star, k_classes=10, epochs=3):
def train_weights_store_misclass(train_new_weights, ds, c_star, k_classes=10, epochs=3):
    """
...
# -- 1st: GET THE WEIGHTS FOR WHOLE DATASET AND `d_star` FROM QUESTION 2 (i.e. 5.55) --------------------------
    ...
    kernel_matrix = shfun.compute_gauss_kern_matrix(x1=x, x2=x, c=c_star)
...
# -- 2nd: MAKE PREDICTIONS WITH TRAINED WEIGHTS, USING WHOLE DATASET AND PRE-COMPUTED KERNEL MATRIX FROM ABOVE -------
    ...
    np.savetxt('../saved_values/Q6/misclass_list.txt', misclass_list, fmt='%d')
...
def get_misclassified_and_plot():
    """
    ...
    misclasses = np.loadtxt('../saved_values/Q6/misclass_list.txt', dtype=int)
...
if __name__ == '__main__':
    ...
    # misclasses = train_weights_store_misclass(train_new_weights=False, ds=ds, d_star=5, k_classes=10, epochs=3)
    # misclasses = train_weights_store_misclass(train_new_weights=True, ds=ds, c_star=0.01, k_classes=10, epochs=3)
    get_misclassified_and_plot()
